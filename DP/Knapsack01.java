// Recursion :  TC-> >> O(2^n) as we are taking same index multiple times, SC-> O(n):recursion stack space
class Knapsack01{
    static int solve(int index, int wt, int[] weights,int[] values){
        ...
        int pick = (int)-1e9;
        if(weights[index] <= wt) pick = values[index] + solve(index-1, wt-weights[index],weights,values);
...
//Memoization :  TC->O(n x bagWeight), SC->O(n):recursion stack space + O(n x bagWeight) : dp array
class Knapsack01{
    static int solve(int index, int wt, int[] weights,int[] values,int[][] dp){
        ...
        int pick = (int)-1e9;
        if(weights[index] <= wt) pick = values[index] + solve(index-1, wt-weights[index],weights,values,dp);
...
class Knapsack01{
    public static void main(String args[]){
        ...
        for(int wt=0;wt<= bagWeight;wt++){
            if(weights[0] <= wt) dp[0][wt] = values[0];
        }
        for(int index=1;index<n;index++){
            for(int wt=0;wt<=bagWeight;wt++){
                int notPick = 0 + dp[index-1][wt];
                int pick = (int)-1e9;
                if(weights[index] <= wt) pick = values[index] + dp[index-1][wt-weights[index]];
...
class Knapsack01{
    public static void main(String args[]){
        ...
        for(int wt=0;wt<= bagWeight;wt++){
            if(weights[0] <= wt) prev[wt] = values[0];
        }
        for(int index=1;index<n;index++){
            for(int wt=0;wt<=bagWeight;wt++){
